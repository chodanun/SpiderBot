...
def main():
	file_items = open("csv/skin-protection/banana-boat.csv", "w")
	file_comment = open("csv/skin-protection/banana-boat-comment.csv", "w")
	item_id = 210
	comment_id = 921
	brand = "Banana Boat"
	type_items = "skin protection"

	url = "http://bananaboat.konvy.com/search?brandDetailId=260&pid=113"
	...
	for i in links :
		...
		while True : # comment table (more than 1 page)
			...
			for k in comments :
				...
				try: # comment title 
					if k.contents[1].get('class')[0] == "right_text_tit" :
						comment_title = k.contents[1].get_text().encode('utf-8').strip().replace("'"," ").replace('"',' ').replace(","," ").replace("\n"," ")
					else :
						comment_title = ""	
				except Exception as e:
					comment_title = ""
			...
			try:
				if next_page.contents[-2].contents[0].get_text().encode('utf-8').strip() == "Next" :
